...
static size_t increamentInstruction(const char* opcode, Chunk* chunk, size_t offset) {
	...
	else return simpleInstruction(opcode, offset) + 1u;
}
...
size_t disassembleInstruction(Chunk* chunk, const size_t offset) {
	...
	switch(ins) {
		...
		case OP_BREAK_JUMP: 
			return jumpInstruction("OP_BREAK_JUMP", chunk, offset, 1);
		...
		case OP_SWITCH_JUMP: 
			return jumpInstruction("OP_SWITCH_JUMP", chunk, offset, 1);
